...
class DbStudentManager {
  ...
  Future openDb() async {
    if (_database == null) {
      _database = await openDatabase(
        ...
        await db.execute(
          "CREATE TABLE student(id INTEGER PRIMARY KEY autoincrement, name TEXT, course TEXT, gender TEXT)",
        );
      } );
    }
  }
  ...
  Future<List<Student>> getStudentList() async {
    ...
    return List.generate(maps.length, (i) {
      return Student(
          id: maps[i]['id'], name: maps[i]['name'], course: maps[i]['course'],gender: maps[i]['gender']);
    });
  }
...
class Student {
  ...
  String course;
  String gender;
  Student({@required this.name, @required this.course, @required this.gender, this.id});
